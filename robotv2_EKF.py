...
class RoverEKF:
    ...
    Replaces the slow Visual Odometry with fast Sensor Fusion.
    """
    def __init__(self, x=0, y=0, theta=0):
        ...
        # 0.01 = uncertainty in wheel speed, 0.005 = uncertainty in gyro
        self.Q = np.diag([0.01, 0.01, 0.005]) 
        
    def predict(self, v_linear, w_gyro, dt):
        """
        Prediction Step: Fuses Wheel Odometry + IMU
        """
        theta = self.state[2]
        ...
        # 2. Covariance Prediction (Jacobian F)
        F = np.eye(3)
        F[0, 2] = -v_linear * np.sin(theta) * dt
        F[1, 2] =  v_linear * np.cos(theta) * dt
...
class OptimizedRoverController:
    ...
    def __init__(self, model, data, mission_name="rover_mission"):
        ...
        # Kalman Filter (EKF)(This is modified)
        # Initialize at (0,0,0) or your starting position
        self.ekf = RoverEKF(x=0.0, y=0.0, theta=0.0)
        self.vio_telemetry = { # We keep this name for compatibility with your plotter
            'vio_position': [],
            'position_error': [],
            'num_features': [] # We will just log 0 here
        }
    ...
    def step(self, sim_time: float, dt: float):
        ...
        # ========== VIO UPDATE (EVERY 10TH FRAME) ==========(This is modified version)
        # ========== EKF UPDATE (EVERY FRAME) ==========
        # 1. Calculate Linear Velocity from Wheel Encoders
        # Average the speed of all 6 wheels
        wheel_radius = 0.05
        avg_wheel_speed = np.mean(state.wheel_velocities)
        v_linear = avg_wheel_speed * wheel_radius

        # 2. Get Angular Velocity from IMU (Gyro Z-axis)
        w_gyro = state.chassis_gyro[2]

        # 3. Update EKF
        # Note: We run this every frame because it is very fast
        estimated_state = self.ekf.predict(v_linear, w_gyro, dt)
        
        # 4. Log Telemetry (mapping EKF data to your existing VIO plots)
        ekf_position = estimated_state[0:3] # x, y, theta (z is ignored)
        
        # For plotting, we construct a 3D vector (x, y, 0)
        # We compare EKF position vs Real Chassis Position
        pos_estimate_3d = np.array([estimated_state[0], estimated_state[1], 0.0])
        position_error = np.linalg.norm(pos_estimate_3d[:2] - state.chassis_pos[:2])

        self.vio_telemetry['vio_position'].append(pos_estimate_3d)
        self.vio_telemetry['position_error'].append(position_error)
        self.vio_telemetry['num_features'].append(0) # No visual features used
        
        # Optional: Print status
        if sim_time % 5.0 < dt:
             print(f" EKF | Pos: ({estimated_state[0]:.2f}, {estimated_state[1]:.2f}) | Error: {position_error:.3f}m")
